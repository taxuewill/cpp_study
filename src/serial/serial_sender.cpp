...
const string kSerialDevice2("/dev/ttyUSB1");
...
{
    // HostSerialInterface * serial_interface = (HostSerialInterface *)interface;
    // HostSerial * serial = (HostSerial *)serial_interface->underlying_instance;
        ...
        if(recv_state == 0) {
            ...
            if(len == 1) {
                if(buf[0] == 0x5A) {
                    ...
                    //TracePrint(GetTrace(gControlDataSync), TraceLevelVerbose, "timeoff: mcu.processtime: %lld\n",((g_sendtime2.tv_sec-g_sendtime.tv_sec)*1000000 +g_sendtime2.tv_usec-g_sendtime.tv_usec));
                }
                else {
                    // TracePrint(GetTrace(gControlDataSync), TraceLevelVerbose, "%s: recv not 5A but is %02x\n",__FUNCTION__,buf[0]);
                    recv_state = 0;
                }
            }
            else {
            ...
            if(len == 1) {
                if(buf[0] == 0xA5) {
                    recv_state = 2;
                    // TracePrint(GetTrace(*serial_interface), TraceLevelVerbose, "recv start: 5A A5\n");
                }
                else {
                    recv_state = 0;
                    // TracePrint(GetTrace(*serial_interface), TraceLevelWarning, "error byte %d\n", recv_state);
            ...
            if(len >= 0) {
                ...
                if(remain_len == 0) {
                    recv_state = 4;
                    // TracePrint(GetTrace(*serial_interface), TraceLevelVerbose, "recv data5:%x %x %x %x %x\n",buf[0],buf[1],buf[2],buf[3],buf[4]);
                }
                else {
                ...
                recv_state = 0;
                // TracePrint(GetTrace(*serial_interface), TraceLevelWarning, "error byte %d, len error %d:%d\n", recv_state, len, remain_len);
            }
        }
            ...
            // if(data_buf == NULL) {
            //     // TracePrint(GetTrace(*serial_interface), TraceLevelWarning, "error byte %d, data null\n", recv_state);
            //     recv_state = 0;
            // }
            // else
            {
                ...
                if(len >= 0) {
                    ...
                    // TracePrint(GetTrace(*serial_interface), TraceLevelWarning,"error byte %d, len short %d:%d\n", recv_state, len, remain_len);
                    recv_state = 0;
                    // free(data_buf);
                    // data_buf = NULL;
                }
            ...
            if(len >= 0) {
                ...
                if(remain_len == 0) {   //todo optimize use buf instead of temp_buffer
                    ...
                    if (checksum == recv_checksum) {
                        ...
                        // TracePrint(GetTrace(*serial_interface), TraceLevelWarning,"Checksum error!\n");
                ...
                // TracePrint(GetTrace(*serial_interface), TraceLevelWarning,"Checksum %d len error %d:%d\n", recv_state, len, remain_len);
...
void ComRead(int n_bytes)
{
    ...
    while(1){
        ...
        int nread = read(fd,read_buffer+read_size,n_bytes+9 - read_size);

        if(nread>0)
        {
            printf("ComRead nread:%d\n",nread);
            for(int i=0;i<read_size;i++){
                printf(" %02x ",read_buffer[i+read_size]);
            }
            printf("\n");
            // read_size -=nread;
        ...
        read_size += nread;//18
        if(read_size == n_bytes+9){
            read_size==0;
            struct timeval tv;
            gettimeofday(&tv,NULL);
            uint64_t time_now = tv.tv_sec * 1000*1000 + tv.tv_usec;
            printf("time_now:%ld,tv_sec:%ld\n",time_now,tv.tv_sec);

            uint64_t time_rcv = 0;
            memcpy(&time_rcv,&read_buffer[4],sizeof(time_rcv));
            printf("time_rcv:%ld\n",time_rcv);
            printf("time_delay:%ld us\n",time_now - time_rcv);
            printf("vector size:%d,index:%d\n",time_delay_list.size(),idx);
            time_delay_list[idx] = (time_now - time_rcv);
            idx++;
            // if(idx == cnt){
            //     //max min  average
            //     int max = 0;int min = 9999; int count = 0;
            //     for(int i=0; i<idx; i++){
            //         count += time_delay_list[i];
            //         if(max < time_delay_list[i]){
            //             max = time_delay_list[i];
            //         }
            //         if(min > time_delay_list[i]){
            //             min = time_delay_list[i];
            //         }
            //     }
            //     count = count/idx;
            //     printf("max:%d,min:%d,avg:%d\n",max,min,count);
            // }
            for(int i = 0;i < n_bytes + 9;i++){
                printf("%02x ",read_buffer[i]);
            }
            printf("\n idx:%d\n",idx);
            printf("crc is %04x\n",GetCrc16(read_buffer+2,n_bytes+5));
            // break;
...
//    printf("count :%d,data_size:%d,hz:%d\n",cnt,data_size,hz);

//    cout<< sizeof(MowerBaseTimestampData)<<endl;
    ...
    // if(!open_port(fd2,kSerialDevice2)){
    //     return 0;
    // }

    // if(HostSerialConfig(fd2,B57600)<0){
    //     close(fd2);
    //     perror("set_opt error");
    //     return 1;
    // }else{
    //     cout<<"set Config success fd:"<<fd2<<endl;
    // }
...
//        printf("send data %02x\n",writedata[0]);
        ...
        gettimeofday(&tv,NULL);
        uint64_t time_now = (tv.tv_sec * 1000*1000 + tv.tv_usec)/1000;
        uint64_t time_begin = (tv.tv_sec * 1000*1000 + tv.tv_usec);

//        memcpy(&writedata[7],&time_now,sizeof(time_now));

        uint16_t checkSum = siCheckSum(writedata + 2, my_size + 5);
//        printf("check sum is %d\n",checkSum);

        memcpy(writedata+my_size+7,reinterpret_cast<uint8_t*> (&checkSum),2);


        int nsend=write(fd,reinterpret_cast<void *>(writedata),my_size+9);

        //    printf("  nsend:%d write fd:%d  \n",nsend,fd);
        ...
        {
            printf("OK  nsend:%d write fd:%d  \n",nsend,fd);
        }

        gettimeofday(&tv,NULL);
        uint64_t time_end = (tv.tv_sec * 1000*1000 + tv.tv_usec);

        n_times++;
        if((n_times%10)==1){
            printf("time_begin:%ld,time_end:%ld\n",time_begin,time_end);
            printf("check sum is %02x %02x\n",writedata[my_size+7],writedata[my_size+8]);
            printf("send data:\n");
            for(int i=0;i<my_size+9;i++){
//                printf(" [%d] %02x ",i,writedata[i]);
                printf("%02x ",writedata[i]);
            }
            printf("\n");
            printf("usleep : %ld\n",(1000000/hz) - (time_end-time_begin));
        }
//        if(n_times>cnt-1)
//            send_flag = false;
        ...
        if(hz>0){
            if((1000000/hz) - (time_end-time_begin)>0){
                usleep((1000000/hz) - (time_end-time_begin));
